var xmlJs = require('xmljs_trans_js'),
  ...
  modifiedResults = function(req, res) {
    ...
    var wrapResponse = function(inData, params) {
      ...
      buildResponse = function(result, format, params, callback) {
        ...
        var indent = req.query.pretty ? 2 : null,
          ...
          formatResponse = {
            ...
            'kml': function() {
                result = wrapResponse(result, params);
                result = osmGeojson.osm2geojson(xmlJs.xmlify(result), true);
                response.contentType = 'application/vnd.google-earth.kml+xml';
                var ogr = ogr2ogr(result).project('EPSG:4326').format('KML');
              ...
              // TODO: A shape export will need to export three files, point, line, polygon
          };

        var ogrResponse = function(proj, format) {
          result = wrapResponse(result, params);
          result = osmGeojson.osm2geojson(xmlJs.xmlify(result));
          // TODO: put this in a lookup?
          var ogr = ogr2ogr(result).project('EPSG:' + proj).format(format);
          ogr.exec(function(err, data) {
            console.log(result);
            if (!err) {
        ...
        if (!formatResponse[format]) {
          if (format === 'ogr' && req.query.ogr) {
            ogrResponse(req.query.proj || '4326', req.query.ogr);
          } else {
            format = 'xml';
            callback(formatResponse[format]());
          }
        } else {
          var resp = formatResponse[format]();
          if (resp) {
            callback(resp);
          }
        }
    ...
    return {
      send: function(inData, format, params) {
        ...
        buildResponse(inData, format, params, function(response) {
      ...
      status: function(error, format, params) {
        ...
        buildResponse(result, format, params, function(response) {
          res.set('Content-Type', response.contentType);
...
exports = module.exports = function(app) {
  return {
    allow: function(method, path, version, auth, callback) {
      addMethod(method, path, version, app, auth, callback);
    }
  };
};

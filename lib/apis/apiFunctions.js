...
exports = module.exports = {
  ...
  readOsmChange: {
    changeset: function(data, callback) {
      ...
      if (data && data.osmChange) { // && data.osmChange.create && data.osmChange.modify && data.osmChange.delete) {
        ...
        } else {console.log('No need to change the changeset id from:', changesetRequest.create.changeset.id);}
  ...
  oauth: function (req, res, oauthConfig, subCallback, callback) {
    var osmOauth = new oauth.OAuth(
      'http://' + oauthConfig.server + '/oauth/request_token',
      'http://' + oauthConfig.server + '/oauth/access_token',

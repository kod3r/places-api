  ...
  'create': {
    'node': function(nodes, callback) {
      ...
      nodes.map(function(node) {
        ...
        // maybe unlink the sequence with the column? ALTER TABLE nodes ALTER COLUMN node_id SET DEFAULT nextval('nodes_id_seq');
        query = queries.insert.all.nodes.join('\n');
        ...
        database().query(query, 'node', function(_, addNode) {
          if (addNode && addNode.error) {
            ...
            // Run all the supplemental queries
            queryList = [];
            queryList.push(queries.insert.current.nodes.join('\n'));
            if (node.tag) {

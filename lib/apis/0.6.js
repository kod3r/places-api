  ...
  changeset: function(data, callback) {
    ...
    runCommand = function(type, change) {
      ...
      if (type === 'node') {
        ...
        change.tag = valToArray(change.tag);
        query = 'SELECT to_json(upsert_node(\'{{id}}\', \'{{lat}}\', \'{{lon}}\', \'{{changeset}}\', \'{{visible}}\', \'{{tag}}\')) AS {{&type&}}';
      } else if (type === 'way') {
        query = 'SELECT to_json(upsert_way(\'{{id}}\', \'{{changeset}}\', \'{{visible}}\', \'{{nd}}\', \'{{tag}}\')) AS {{&type&}}';
      } else if (type === 'relation') {
        query = 'SELECT to_json(upsert_relation(\'{{id}}\', \'{{changeset}}\', \'{{visible}}\', \'{{member}}\', \'{{tag}}\')) AS {{&type&}}';
      }
    ...
    processRequests = function(type) {
      ...
      functionList = [];
      reassignNodes(changesetRequest, returnData);
    ...
    reassignNodes = function(inputData, replaceData) {
      var translationTable = {}, wayTranslationTable = {};
      if (replaceData && replaceData.node) {
        replaceData.node.map(function(newNode) {
          translationTable[newNode.old_id] = newNode.new_id;
        });
      }
      if (replaceData && replaceData.way) {
        replaceData.way.map(function(newWay) {
          wayTranslationTable[newWay.old_id] = newWay.new_id;
        });
      }

      // Loop through that ugly object and update the nodes!
      for (var action in changesetRequest) {
        if (changesetRequest[action].way) {
        ...
        }  else if (changesetRequest[action].relation) {

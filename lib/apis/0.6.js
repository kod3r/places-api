  ...
  if (req.query[types] && !isNaN(req.query[types].replace(/,/g, ''))) {
    ...
    typeList.map(function(typeId, typeIndex) {
      if (typeId && !isNaN(typeId)) {
        req.params[type + typeIndex] = typeId;
    ...
    query.pop();
    query = query.join('\n');
    database(req, res).query(query, type, respond);
  } else {
...
},
{
  ...
  'path': 'nodes',
  'process': function(req, res) {
    queryMultipleElements(req, res, 'node');
  }
},
{
  'name': 'GET way/#id',
...
},
{
  ...
  'process': function(req, res) {
    ...
    database(req, res).query(queryList, null, respond);
  }
},
{
...
},
{
  ...
  'path': 'ways',
  'process': function(req, res) {
    queryMultipleElements(req, res, 'way');
  }
},
{
...
},
{
  ...
  'path': 'relations',
  'process': function(req, res) {
    queryMultipleElements(req, res, 'relations');
  }
},
{

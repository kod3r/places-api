...
},
{
  ...
  'process': function(req, res) {
    ...
    //TODO: Make this an upsert task
    apiFunctions.readXmlReq(req, function(error, data) {
      if (error) {
        apiFunctions.respond(res,{'error': {'code': 503, 'description': 'PUT Data Read Error'}});
      } else {
        ...
        if (data.osm && data.osm && data.osm.changeset) {
          ...
          database(res, req).query(query, 'id', function(x, result) {
            ...
            if (data.osm.changeset.tag) {
              var tags = apiFunctions.valToArray(data.osm.changeset.tag);
              for (var tagIndex in tags) {
                // TODO: get rid of this SQL injection fun time
          ...
          apiFunctions.respond(res,{'error': {'code': 502, 'description': 'Read Error', 'details': '3'}});
        }
      }
    });
  }
...
},
{
  ...
  'process': function(req, res) {
    // JOSM uses this
    // This should use the changeset upsert command, maybe roll it into the big changeset command?
    // This code is here as a placeholder!
    var query = queries.select.all.changesets.concat('WHERE', queries.where.changeset.id).join('\n');
    database(req, res).query(query, 'changeset', apiFunctions. respond);
  }
},

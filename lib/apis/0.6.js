...
readOsmChange = {
  changeset: function(data, callback) {
    ...
    reassignNodes = function(type, changeset, referenceData) {
      //TODO: use changeset and referenceData instead of the actual variables
      //// So we can move this out of the function if we want
      ...
      for (var action in changeset) {
        if (type === 'way' && changeset[action].way) {
          changeset[action].way = valToArray(changeset[action].way);
          for (var way in changeset[action].way) {
            if (changeset[action].way[way].nd) {
              changeset[action].way[way].nd = valToArray(changeset[action].way[way].nd);
        ...
        if (type === 'relation' && changeset[action].relation) {
          changeset[action].relation = valToArray(changeset[action].relation);
          for (var relation in changeset[action].relation) {
            if (changeset[action].relation[relation].member) {
              changeset[action].relation[relation].member = valToArray(changeset[action].relation[relation].member);
...
},
{
  ...
  'path': 'nodes',
  'process': function(req, res) {
    var query = queries.select.current.nodes.concat('WHERE', 'api_current_nodes.id IN (\'{{nodes}}\')').join('\n');
...
},
{
  ...
  'path': 'ways',
  'process': function(req, res) {
    var query = queries.select.current.ways.concat('WHERE', 'api_current_ways.id IN (\'{{ways}}\')').join('\n');
...
},
{
  ...
  'path': 'relations',
  'process': function(req, res) {
    var query = queries.select.current.relations.concat('WHERE', 'api_current_relations.id IN (\'{{relations}}\')').join('\n');
...
},
{
  ...
  'process': function(req, res) {
    //TODO: Make this more list the map function
...
},
{
  ...
  },
  {
    ...
    'process': function(req, res) {
      ...
      // TODO: implement this for all queries from database view

...
readOsmChange = {
  ...
  'create': {
    'node': function (nodes, callback) {
      var query, currentQuery, returnData = [], counter = 0;
      var loopCallback = function(data) {
      ...
      nodes.map(function(node) {
        ...
        //console.log(node);
        query = database().addParams('INSERT INTO {{&type&}}s (latitude, longitude, changeset_id,visible, timestamp, tile, version) VALUES (\'{{lat}}\', \'{{lon}}\', \'{{changeset}}\', true, now(), \'-1\', \'1\') returning {{id}} as old_id, node_id as new_id, version as new_version', 'node', node);
        database().query(query, 'node', function(_, addNode) {
          ...
          if (node.tag) {
            valToArray(node.tag).map(function(tag) {
              query = database().addParams('INSERT INTO node_tags (node_id, version, k, v) VALUES (\'{{new_id}}\', \'{{new_version}}\', \'{{k}}\', \'{{v}}\')', 'node', node);
              query = database().addParams(query, 'node', tag);
              database().query(query, 'node', function() {
                query = database().addParams('INSERT INTO current_node_tags (node_id, k, v) VALUES (\'{{new_id}}\', \'{{k}}\', \'{{v}}\')', 'node', node);
                query = database().addParams(query, 'node', tag);
          ...
          currentQuery = database().addParams('INSERT INTO current_{{&type&}}s (SELECT {{&type&}}_id as id, latitude, longitude, changeset_id, visible, timestamp, tile, version FROM {{&type&}}s WHERE {{&type&}}_id = \'{{new_id}}\' AND version = \'{{new_version}}\')  returning {{id}} as old_id, id as new_id, version as new_version', 'node', node);
          database().query(currentQuery, 'node', function(_, currentData) {
            loopCallback(currentData);
          });
        });
      });
    },
...
},
{
  ...
  'process': function(req, res) {
    readXmlReq(req, function(error, data) {
      readOsmChange.changeset(data, req.params.id, function(result) {
        if (result.data) {

...
readOsmChange = {
  changeset: function(data, callback) {
    ...
    functionList = [],
    returnData = {},
    insertData = function(type, change) {
      ...
      query = 'SELECT to_json(upsert_node(\'{{id}}\', \'{{lat}}\', \'{{lon}}\', \'{{changeset}}\', \'{{tag}}\')) AS {{&type&}}';
      query = database().addParams(query, type, change);

      database().query(query, type, function(_, addNode) {
        if (addNode && addNode.data && addNode.data[type]) {
          console.log('****', JSON.stringify(addNode));
          returnData[type] = returnData[type] ? returnData[type] : [];
          returnData[type].push(addNode.data.node[0].node);
          deferred.resolve(addNode);
        } else {
          deferred.reject(addNode);
        }
    ...
    if (data && data.osmChange && data.osmChange.create && data.osmChange.modify && data.osmChange.delete) {
      ...
      // First we do the nodes
      //console.log(changesetRequest.create);
      changesetRequest.create.node = valToArray(changesetRequest.create.node);
      //console.log(JSON.stringify(changesetRequest.create));
      changesetRequest.create.node.map(function(change) {
//        console.log(change)
        functionList.push(insertData('node', change));
      });
/*
      changesetRequest.create.map(function(changes) {
        for (var type in changes) {
          typeChanges = valToArray(changes[type]);
          for (var change in typeChanges) {
            functionList.push({'type': type, 'change': typeChanges[change]});
          }
        }
      });*/

      //console.log(functionList);
      Q.all(functionList).then(function(){callback({'data': returnData});});
...
readOsmChange_old = {
  changeset: function(data, changesetId, callback) {
    console.log('*******************************************');
    console.log(JSON.stringify(data));
    console.log('*******************************************');
    var changeType, objType, changes = 0, completeChanges = [],
    changeTracker = function(change) {
      completeChanges.push(change);
      //console.log(completeChanges.length, changes);
      if (completeChanges.length >= changes) {
        // All requests are complete
        // Create a diffResult suitable of sending back to the browser
        callback(change);
      }
    };
    if (data && data.osmChange) {
      for (changeType in data.osmChange) {
        for (objType in data.osmChange[changeType]) {
          if (readOsmChange[changeType] && readOsmChange[changeType][objType]) {
            changes++;
            readOsmChange[changeType][objType](valToArray(data.osmChange[changeType][objType]), changeTracker);
          } else {
          }
        }
      }
    } else {
      callback({'error': {'statusCode': 400, 'description': 'Errors Parsing XML'}});
    }
  },
  'create': {
    'node': function (nodes, callback) {
      var query;
      nodes.map(function(node) {
        node.lon = Math.round(node.lon * 10000000);
        node.lat = Math.round(node.lat * 10000000);
        node.tag = valToArray(node.tag);
        query = 'SELECT to_json(upsert_node(\'{{id}}\', \'{{lat}}\', \'{{lon}}\', \'{{changeset}}\', \'{{tag}}\')) AS node';
        query = database().addParams(query, 'node', node);
        database().query(query, 'node', function(_, addNode) {
          // Fix to bring the 'to_json' function down one level
          if (addNode && addNode.data && addNode.data.node && addNode.data.node[0] && addNode.data.node[0].node) {
            addNode.data.node = addNode.data.node[0].node;
          }
          callback(addNode);
        });
      });
    },
    'node_old': function(nodes, callback) {
      var query, queryList = [], counter = 0, responses, returnData = [];

      var loopCallback = function(_, data) {
        // TODO: Error checking
        console.log(data);
        if (data && data.data && data.node) {
          returnData.push(data.data.node[0]);
        } else {
          console.log('error', data, returnData);
          returnData.push(null);
        }
        if (returnData.length >= counter) {
          callback({data: { node: returnData}});
        }
      };

      nodes.map(function(node) {
        // Make the lat/lon compatible with OSM
        counter++;
        node.lon = Math.round(node.lon * 10000000);
        node.lat = Math.round(node.lat * 10000000);

        // Add the node and return its new node_id
        // TODO: check for an existing node before updating
        query = queries.select.all.nextval.join('\n');
        query = database().addParams(query, 'node', node);
        database().query(query, 'node', function(_, addNode) {
          if (addNode && addNode.error) {
            loopCallback(null, addNode);
          } else {
            node.new_id = addNode.data.node[0].new_id;
            node.new_version = addNode.data.node[0].new_version;
            database().query(queries.insert.all.nodes.join('\n'), 'node', function() {
              // Run the supplemental queries
              queryList = [];
              if (node.tag) {
                valToArray(node.tag).map(function(tag) {
                  // TODO: in the first step, figuring out if we're updating or adding a new node
                  queryList.push(database().addParams(queries.insert.all.nodeTags.join('\n'), 'node', tag));
                  queryList.push(database().addParams(queries.insert.current.nodeTags.join('\n'), 'node', tag));
                });
              }
              responses = multiRespond(queryList, loopCallback);
              for (var queryIndex in queryList) {
                database().query(database().addParams(queryList[queryIndex], 'node', node), 'node', responses);
              }
           });
          };
        })
    ...
    'way': function (ways, callback) {
    },
    'relations': function (ways, callback) {
    }
  },
  'modify': {
    'node': function(nodes, callback) {return readOsmChange.create.node(nodes, callback);}
  },
  'delete': {
  }
};
    ...
    'process': function(req, res) {
      ...
      },
      {
        ...
        'query': queries.select.current.nodes.concat(
          ...
          ') ways_in_bbox ON current_way_nodes.way_id = ways_in_bbox.way_id',
          'UNION',
          queries.select.current.nodesInBbox,
          ') nodes_in_way ON current_nodes.id = nodes_in_way.node_id'
        ).join('\n')
      }, {
        'type': 'way',

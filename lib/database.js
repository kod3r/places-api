...
exports = module.exports = function(req, res) {
  ...
  var databaseTools = {
    ...
    addParams: function (query, type, params) {
      ...
      /*cleanPgsql = function(val) {
        if (val) {
          //make val into a string
          val = val.toString();
      ...
      return {query: newQuery, queryParams: paramArray};
    },
      ...
      databaseTools.database(function(err, client, done) {
        if (err) {
          ...
          if (typeof(query) === 'object' && query.query) {
            ...
            paramQuery = databaseTools.addParams(query, type);
          }
          client.query(paramQuery.query, paramQuery.queryParams, function(err, results) {
            if (err) {
              queryResult.error = {'code': '404'};
              queryResult.details = err;
              queryResult.details.query = paramQuery.query;
              queryResult.details.paramArray = paramQuery.queryParams;
            } else {

...
exports = module.exports = [{
  ...
  'process': function (req, res) {
    ...
    console.log(auths);
    osmOauth.getOAuthAccessToken(auths[authName.token],keys[auths[authName.token]][authName.key], function (error, oauthAccessToken, oauthAccessTokenSecret) {
      if (error) {
        res.status(error.statusCode || 500);
        res.send(error);
      } else {
        ...
        osmOauth.get(
          ...
          function (e, data){
            ...
            keys[auths[authName.token]].userName = jsonData.osm.user.display_name;
            keys[auths[authName.token]].id = jsonData.osm.user.id;
            keys[auths[authName.token]].userXml = data;
            res.send([
              'oauth_token=', oauthAccessToken,
              '&oauth_token_secret=', oauthAccessTokenSecret,
              '&username=', keys[auths[authName.token]].userName,
              '&userId=', keys[auths[authName.token]].id
            ].join(''));
          });
      }
    });

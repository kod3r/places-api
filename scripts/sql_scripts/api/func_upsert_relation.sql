CREATE OR REPLACE FUNCTION upsert_relation(
  ...
  BEGIN
    ...
    -- Update the pgsnapshot view
        SELECT res FROM nps_dblink_pgs('select * from pgs_upsert_relation(' || quote_literal(v_new_id) || ', ' || quote_literal(v_changeset) || ', ' || quote_literal(v_visible) || ', ' || quote_literal(v_members) || ', ' || quote_literal(v_tags) || ', ' || quote_literal(v_timestamp) || ', '  || quote_literal(v_new_version) || ', ' || quote_literal(v_user_id) || ')') into v_res;
